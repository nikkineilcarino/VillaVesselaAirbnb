...
Phase 6 production state: an owner-approved Supabase project is linked locally through ignored CLI state. Migrations `001` through `008` were applied remotely in order on 2026-08-10, remote migration history matches, and linked schema lint reports no errors. No synthetic seed data was applied. Migration `008` implements distinct Waze reporting and analytics-only daily deletion once events are older than 365 days; inquiry retention/deletion remains unapproved.
...
Phase 8 production state: consent-based anonymous analytics is enabled. Before Allow, after Decline, and on administrator routes, no analytics identifier/request exists. One live `/contact` page view and approved Airbnb Contact click returned `201`, matched exact Supabase rows, and were deleted exactly after reporting QA. Waze is reported separately. The process-local rate limiter remains non-atomic across serverless instances, so a privacy-compatible distributed/WAF control is still advisable for sustained or adversarial traffic.

Phase 9 production state: the approved administrator can open the RLS-backed Analytics dashboard and Inquiries page. Empty-state activation QA and a later populated synthetic run both passed. Operational status, six summary cards, all nine link categories, five chart tables, recent activity, Asia/Manila range handling, refresh, RPC values, and page/link CSV exports reconciled exactly. Production-authenticated mobile tooltip/legend interaction remains a narrow manual follow-up; the accessible chart tables and credential-independent responsive coverage pass.

Phase 10 interim state: the inquiry form, endpoint, administrator list/status workflow, and protected exports are implemented but unpublished. `CONTACT_INQUIRY_VISIBLE` and `CONTACT_INQUIRY_ENABLED` remain false, so guest/admin inquiry UI is absent and direct protected inquiry routes/exports fail closed. Enabled-visible, published-disabled rollback, and unfinished-hidden browser modes are tested, but no live inquiry insertion, inquiry status mutation, or inquiry CSV handling has been exercised. Page-view and link-click CSV exports were verified separately during analytics activation.

Phase 11 state: the Privacy page, metadata, approved photo-based social card, sitemap, fail-closed robots, manifest/icons, conservative structured data, accessibility hardening, performance review, and security headers are implemented. Production indexing is enabled only for the final public HTTPS origin. The structured data deliberately omits property images, exact coordinates, map/contact destinations, prices, conditional capacity, and other unresolved facts. Explicit Allow/Decline/settings control, analytics-only retention, and the configured Contact privacy-request route are active. Responsible handling, inquiry retention/deletion, provider review, and jurisdiction-specific obligations remain operational decisions rather than invented claims.

Phase 12 release state: the public site is deployed at `https://villa-vessela-airbnb.vercel.app`, which is the configured canonical origin. GitHub and Vercel publication, indexing/header/accessibility/privacy smoke checks, and private-contact/browser-secret scans pass. The Airbnb listing, Facebook page, Messenger conversation, WhatsApp contact, Google Maps/Waze pin, Nida's caretaker telephone, and public email are approved public destinations. Supabase production Auth/database configuration, migrations `001` through `008`, one approved owner administrator, a Production-only backend secret, and consent-based analytics are active. Inquiry submission and retained test credentials remain absent. The remaining photo gaps, owner telephone destination, rates, conditional inclusions, inquiry retention/deletion, and privacy operations remain unresolved or explicitly bounded.

- [x] Keep the unfinished website inquiry feature hidden from guests and administrators until its controlled publication/activation steps (owner instruction, 2026-08-31).
- [ ] Confirm whether English-only content is desired or English/Filipino language switching is required.
- [x] Approve and implement 365-day analytics-only retention with daily deletion on 2026-08-10.
- [x] Approve and implement the separate 365-day inquiry retention/exact-deletion process locally; remote application and live proof remain gated.
- [x] Use the configured public Contact route as the analytics privacy-request channel.
- [ ] Assign the responsible person, response process, and verified privacy-request deletion procedure.
- [x] Require explicit Allow/Decline analytics choice plus a persistent settings control before production collection.
...
- [x] Create the owner-approved Supabase project, apply and lint migrations `001` through `008`, configure Auth/RLS public values, and add the backend secret only as a sensitive Production server variable on 2026-08-10.
...
- [ ] Provide retained non-production approved and unapproved accounts through ignored local/CI secret storage; production QA used disposable identities that were deleted exactly.
- [x] Complete isolated live page-view/link-click insertion, approved-admin read/reporting, unapproved denial, and exact cleanup against the approved production project on 2026-08-10.
- [ ] Before sustained or adversarial traffic, approve a distributed/serverless analytics rate-limit provider or Vercel WAF policy that does not persist raw IP addresses.
