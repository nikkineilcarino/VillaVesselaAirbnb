...
- **Consequences:** analytics requires both an exact `true` build/server flag and an explicit visitor Allow choice; disabled, declined, or failed storage does not break the site. Process-local limits do not provide globally atomic serverless enforcement, so a privacy-compatible distributed limiter/WAF remains advisable for sustained or adversarial traffic. No raw IP is retained to compensate. Production activation and retention are governed by Decision 037.
- **Date:** 2026-07-23
...
## Decision 037 — Activate first-party analytics only with explicit choice, bounded retention, and exact live proof

- **Context:** The owner approved repairing the empty production analytics dashboard. The existing Supabase system was intentionally disabled, its backend write credential was absent, Waze was not represented accurately, visitors had no explicit analytics preference, retention was not operational, and populated reporting had never been reconciled against stored rows.
- **Options considered:** install Vercel or third-party analytics; enable the existing endpoints without a preference/retention boundary; keep analytics permanently disabled; activate the existing first-party design after consent, schema, retention, reporting, rollback, and exact live acceptance gates pass.
- **Selected approach:** retain the custom Next.js/Supabase analytics architecture; require an explicit Allow/Decline choice before identifier or event creation; provide persistent settings; keep identifiers random and fields minimized; add distinct Waze reporting; apply migration `008` with one daily analytics-only 365-day pruning job; configure one modern full-privilege backend secret as sensitive and Production-only; expose safe storage/reporting status and refresh to approved administrators; keep inquiry submission false; and require one isolated page-view/approved-link pair to return `201`, reconcile through rows/RPCs/dashboard/page-link CSV, and be deleted exactly.
- **Reason:** this makes the requested dashboard operational without adding a paid/third-party tracker, identifying visitors, granting public database access, blocking native contact navigation, or presenting synthetic QA data as real traffic. Independent consent, server validation, RLS-bound reporting, and exact cleanup make each boundary observable.
- **Consequences:** production uses `ANALYTICS_ENABLED=true` and must be rebuilt after a flag change. Before Allow, after Decline, on administrator routes, or when the feature is false, no analytics identifier/request exists. The backend secret bypasses RLS and therefore depends on Production-only isolation, two active validated analytics handlers, and one separately reviewed default-disabled inquiry handler; administrator reads never use it. Analytics is deleted daily once older than 365 days, but the schedule or a paused project can delay execution. In-process limits are accepted for the current low-volume site but are not globally atomic; distributed/WAF hardening remains advisable before sustained or adversarial scale. Inquiry activation still requires a separate owner-approved retention, operator, live workflow, and deletion gate. A regression in consent, write health, reporting, or privacy requires setting analytics false and deploying a fresh build. This decision supersedes the disabled-analytics production-state consequences of Decision 026 and the pre-activation WAF wording of Decision 021 while preserving their minimization and failure-isolation rationale.
- **Date:** 2026-08-10

## Decision 038 — Bound inquiry intake with retry identity, retention, and exact administrator deletion

- **Context:** The owner approved a controlled website-inquiry activation, but the default-disabled implementation did not yet define automatic inquiry expiry, an early-deletion workflow, durable retry deduplication, or consent-notice provenance. Inquiry rows contain voluntary personal data, the existing Production-only backend secret has full database privilege because active analytics shares it, and the current abuse limits are process-local rather than globally atomic.
- **Options considered:** retain inquiry rows indefinitely and rely on manual cleanup; treat inquiries as booking records; add new notification, CAPTCHA, or distributed-limiter providers before activation; activate only after an additive schema and operating boundary covers retry identity, privacy-notice provenance, exact deletion, retention, monitoring, and rollback.
- **Selected approach:** migration `009` adds a unique, required UUID-v4 `submission_id` and required `privacy_notice_version`, with no future-insert defaults. Existing pre-version rows, if any, receive a random legacy retry identity and the honest `legacy-unversioned` notice sentinel. The enabled form reuses one UUID v4 across retries for a logical submission and sends the notice version rendered with that form only as a freshness assertion. Strict server validation requires the current notice version, drops the visitor-supplied assertion from the validated storage shape, and supplies the trusted constant to the service-role-only `public.store_contact_inquiry(...)` function. That fixed-search-path function hard-codes consent, `new` status, and server time and returns only `created`, `duplicate`, or `conflict`, never row data. An identical retry succeeds idempotently; the same ID with changed intake fields returns a conflict without overwriting or masquerading as the original. The public handler streams at most 8 KiB of actual request bytes, requires an exact trusted `Origin`, applies bounded rate limits, and never reports unavailable storage as success. Direct inquiry-table DELETE remains unavailable to browser roles. The protected exact-deletion action repeats approved-administrator authorization, validates one UUID plus an explicit confirmation, and calls only `public.delete_contact_inquiry(uuid)` through the authenticated request-scoped client; that fixed-search-path function independently checks `private.is_approved_admin()` and can delete at most one primary-key row. Migration `009` also schedules the distinct daily `villa-vessela-inquiry-retention` job to invoke the parameter-free, owner-only `private.prune_expired_inquiries()` function, which deletes `contact_inquiries` rows strictly older than 365 days from `created_at`, regardless of status. The inquiry job remains independent from analytics retention, and application roles cannot execute pruning. The existing approved owner administrator checks `/admin/inquiries` daily; this activation adds no email, SMS, push, autoresponder, or response-time promise.
- **Reason:** an inquiry is a short-lived request for contact, not availability confirmation, a booking, or a payment record. A stable random retry identifier prevents a lost success response from creating duplicate rows, while the explicit conflict result prevents an edited retry from being misclassified as received. Comparing the rendered notice assertion to the current server version makes a stale tab fail closed, and persisting only the server-selected constant preserves storage authority. Automatic retention plus exact confirmed early deletion gives the active table a bounded lifecycle without adding another data processor.
- **Consequences:** a preferred stay can occur after its inquiry row expires; confirmed booking and payment records therefore remain with the approved booking channel. Deleting the active row does not guarantee immediate erasure from provider backups, browser autofill, downloaded CSV files, or copies moved to an external contact channel; operators must securely remove those separate copies when applicable. CSV exports remain personal data. The backend secret still bypasses RLS and is not made insert-only by database grants, so its effective narrowness depends on Production-only isolation, the service-only storage function, and reviewed handler call sites; it must never be used to prove administrator authorization. Honeypot, timing, and per-client/per-instance limits remain appropriate only for the site's current low volume and require daily monitoring plus immediate disablement on spam or cost escalation; no raw IP, CAPTCHA, paid WAF, notification service, or new third party is added by this decision. Steps 1–2 remain local-only: Production `CONTACT_INQUIRY_ENABLED` stays false and the remote production database stays on migrations `001`–`008` until the later controlled database, dormant-deployment, live-workflow, cleanup, rollback, and activation gates pass. This is an engineering operating decision, not a claim of legal compliance.
- **Date:** 2026-08-24

## Decision 039 — Keep unfinished inquiry surfaces unpublished

- **Context:** After the complete local inquiry gate passed but before commit, migration, or activation, the owner explicitly requested that guests and administrators not see the unfinished website-inquiry feature or its disabled-state messaging.
- **Options considered:** replace the disabled sentence with different copy; hide only the guest form; make visibility identical to collection; add a separate server-only publication boundary while preserving a visible-disabled rollback mode.
- **Selected approach:** add fail-closed `CONTACT_INQUIRY_VISIBLE`, separate from `CONTACT_INQUIRY_ENABLED`. Missing/false visibility plus disabled collection removes inquiry content from public Contact and Privacy, administrator navigation/dashboard/reporting links, recent-row queries, and loading UI; approved direct inquiry pages, mutations, and inquiry CSV exports return `404`/not found. The storage API remains controlled only by `CONTACT_INQUIRY_ENABLED` and continues to return private/no-store `404`. Enabled collection always implies visibility in code so the site cannot collect through a hidden interface. Setting visibility true while collection remains false preserves the reviewed disabled form, Privacy disclosures, administrator access, and rollback mode after publication. The displayed Privacy revision and stored consent provenance share the same trusted server constant.
- **Reason:** an unfinished feature should not advertise itself, but hiding it must not weaken API denial, erase the post-activation privacy/admin lifecycle, or make a future rollback conceal retained personal records.
- **Consequences:** Production keeps both inquiry flags false/absent until the controlled release reaches its publication gate. A dormant pre-activation deployment must remain hidden. Activation sets visibility true only after migration and administrator prerequisites pass, then enables collection in the separately verified live step. After any post-activation rollback, collection becomes false while visibility stays true until all retained inquiry duties are complete. CI and local QA must cover enabled-visible, published-disabled, and unfinished-hidden modes independently.
- **Date:** 2026-08-31
