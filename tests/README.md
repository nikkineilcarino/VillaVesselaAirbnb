...
- `unit/database-schema.test.ts` verifies ordered/documented migrations, required tables/constraints/indexes, deny-by-default RLS/policy structure, RLS-aware Asia/Manila views, and synthetic repeatable seed safeguards.
...
- `unit/analytics.test.ts` verifies path/referrer/category minimization, inactivity rotation, destination normalization, strict schemas, exact allowlisting, body bounds, bounded rate limiting, delivery fallback, and no raw-IP/payload logging.
...
- `e2e/homepage.spec.ts` verifies required homepage sections, important facts and qualifiers, inactive unverified destinations, local supplied photography, the remaining location placeholder, and working About/location anchors.
...
- `e2e/discovery-contact.spec.ts` verifies public access/navigation, all 37 gallery photographs and three open slots, lightbox focus/keyboard/image-fit behavior, review provenance, Messenger placeholders, clipboard copying, inactive map/unapproved contact destinations, exact configured caretaker telephone links, the disabled no-action inquiry preview, mobile overflow, and Axe results on all four routes.
...
- `e2e/analytics.spec.ts` verifies browser UUID storage/lifetimes, referrer minimization, route deduplication, admin exclusion, endpoint bounds/allowlisting, and browsing/navigation during delivery failure.
...
- `e2e/seo-privacy-security.spec.ts` verifies all public canonical/social/JSON-LD output, sitemap/robots/manifest/social/icon routes, Privacy content/reflow/Axe, focus visibility, reduced motion, security headers, and asset caching.
