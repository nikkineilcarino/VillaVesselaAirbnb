...
export async function readBoundedAnalyticsJson(request: Request): Promise<BoundedJsonResult> {
  const contentType =
  ...
  if (contentLength) {
    ...
    if (!Number.isFinite(parsedLength) || parsedLength < 0 || parsedLength > MAX_ANALYTICS_BODY_BYTES) {
...
export function analyticsResponse(status: number) {
