...
The public site helps families and groups understand the property and can continue to an approved booking or contact channel. A separate Supabase-backed administrator area is active for one manually approved owner identity, privacy-safe aggregate analytics, and optional inquiry management. Analytics collection and public inquiry submission remain disabled; public visitors never need an account.
...
Phase 12 public release is complete. The source is published on [GitHub](https://github.com/nikkineilcarino/VillaVesselaAirbnb), and the production site is live at [villa-vessela-airbnb.vercel.app](https://villa-vessela-airbnb.vercel.app). The Node 22 Vercel release passes provider builds and production Chromium checks covering public routes, mobile/keyboard/dialog behavior, Axe, canonical/social/structured metadata, discovery assets, security headers, Privacy, and administrator authorization. The local gate passes lint, strict types, 69 unit tests, the credential-independent browser suite, the separate enabled-inquiry branch, lockfile simulation, and production build. Production live-auth checks pass approved sign-in/dashboard/inquiry access/logout and authenticated-unapproved denial. The production and complete dependency audits are clean. Compatible patched releases cover the modern and legacy brace-expansion branches as well as related PostCSS/nanoid and js-yaml advisories; see [`docs/qa/dependency-audit-2026-07-27.md`](docs/qa/dependency-audit-2026-07-27.md). Approved public destinations remain validated through Vercel environment configuration and absent from Git where appropriate. Supabase migrations `001` through `007` are applied and lint-clean, with one manually approved administrator. Analytics remains an explicit no-op and Contact submission remains unavailable because both production feature flags are false; no service-role key or test credential is deployed.
...
The read-only [GitHub Actions quality workflow](.github/workflows/quality.yml) repeats the locked install, production dependency audit, lint, strict type check, 69 unit tests, production build, and credential-independent Chromium suite on every push and pull request to `main`. It receives no Supabase, administrator, production, or private-contact secret; the two live administrator tests therefore remain explicitly skipped in CI and run only through an approved out-of-band production/non-production procedure.
...
The database layer supplies local Supabase configuration, eight versioned migrations, synthetic seed, typed schema/client factories, and detailed setup in `supabase/README.md`. Migrations `001` through `007` are currently applied in production; pending migration `008` adds distinct Waze reporting and analytics-only retention without activating collection. Migration `007` provides the authenticated, `SECURITY INVOKER` dashboard functions required by `/admin/dashboard`. Start Docker Desktop, run `npm run db:start`, then `npm run db:reset` and `npm run db:lint`. No administrator identity is seeded. Create an Auth user only through an approved administrative path, then provision its matching `admin_profiles` row through trusted SQL/service access as documented in `src/lib/auth/README.md` and `supabase/README.md`. Authentication alone never grants administration, signup remains disabled, and no password or project credential belongs in source.
...
- The Privacy page is implemented, but the pending analytics-retention migration is not applied or live-tested in production yet. Inquiry retention/deletion, submission, status management, and CSV exports remain separate, default-disabled work requiring approved procedures and live verification.
