...
Phase 12 public release is complete. The source is published on [GitHub](https://github.com/nikkineilcarino/VillaVesselaAirbnb), and the production site is live at [villa-vessela-airbnb.vercel.app](https://villa-vessela-airbnb.vercel.app). The final Node 22 Vercel deployment passed its provider build and production Chromium checks covering all public routes, mobile/keyboard/dialog behavior, Axe, canonical/social/structured metadata, discovery assets, security headers, Privacy, and administrator denial. The local gate passes lint, strict types, 67 unit tests, credential-independent browser tests with 2 live administrator checks explicitly skipped, the separate enabled-inquiry branch, lockfile simulation, and production build. The production dependency audit remains clean. A July 27 re-audit found one newly published denial-of-service advisory propagated through nine development-only ESLint package entries; the compatible modern branch is patched, while the legacy branch has no compatible upstream fix yet. The reviewed disposition is documented in [`docs/qa/dependency-audit-2026-07-27.md`](docs/qa/dependency-audit-2026-07-27.md). The owner subsequently approved the Airbnb listing, Facebook page, and two source-package caretaker telephone contacts for public use. All four destinations are validated through Vercel environment configuration; the caretaker values remain absent from Git history. Scans continue to find zero privileged browser markers, active unapproved external links, or analytics cookies. Analytics is an explicit no-op and Contact submission is unavailable because both production feature flags are false and no Supabase credential was configured. Database-backed role, insertion, dashboard, inquiry, export, retention, and deletion checks remain blocked rather than passed.
...
- Every item in the current primary navigation is available, and Privacy is available from the footer. The approved Airbnb listing, Facebook page, and two caretaker telephone contacts can be active from validated environment configuration; Messenger, WhatsApp, map, owner-phone, email, and other external destinations remain disabled because no complete approved value has been supplied.
...
- The Airbnb, Facebook, and caretaker telephone production destinations are supplied. Supabase credentials and administrator test credentials have not been supplied. Docker's installed engine is not running, so migrations/policies/functions, live analytics insertion, populated dashboard rendering, and approved/unapproved authentication behavior have not been executed against a database.
...
The remaining publishing blockers include approved Messenger/Google Maps destinations, public owner contact details, Blue/Green Kubo and confirmed parking photos, a higher-resolution front-of-villa hero, rates and extra fees, exact expanded capacity, kubo and beach-cottage inclusion, the lost-key fee, bathroom layout, washer availability, inquiry-form status, and language choice. See `CONTENT_TODO.md` for the authoritative list.
